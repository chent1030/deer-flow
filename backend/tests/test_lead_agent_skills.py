...
def test_make_lead_agent_empty_skills_passed_correctly(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "_load_enabled_skills_for_tool_policy", lambda available_skills, *, app_config, **kwargs: [])
...
def test_make_lead_agent_intersects_agent_and_visible_skills(monkeypatch):
    from unittest.mock import MagicMock
    ...
    monkeypatch.setattr(lead_agent_module, "build_middlewares", lambda *args, **kwargs: [])
    monkeypatch.setattr(lead_agent_module, "create_agent", lambda **kwargs: kwargs)
    monkeypatch.setattr(lead_agent_module, "load_agent_config", lambda x, **kwargs: AgentConfig(name="test", skills=["allowed", "hidden"]))
    ...
    lead_agent_module.make_lead_agent(
        {
            "configurable": {
                "agent_name": "test",
                "visible_skills": ["allowed"],
            }
        }
    )

    assert captured_skills[-1] == {"allowed"}
...
def test_make_lead_agent_filters_tools_from_available_skills(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "_load_enabled_skills_for_tool_policy", lambda available_skills, *, app_config, **kwargs: [_make_skill("restricted", ["read_file"]), _make_skill("legacy", None)])
...
def test_make_lead_agent_all_legacy_skills_preserve_all_tools(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "create_agent", lambda **kwargs: kwargs)
    monkeypatch.setattr(lead_agent_module, "load_agent_config", lambda x, **kwargs: AgentConfig(name="test", skills=None))
    monkeypatch.setattr(lead_agent_module, "_load_enabled_skills_for_tool_policy", lambda available_skills, *, app_config, **kwargs: [_make_skill("legacy", None)])
...
def test_make_lead_agent_denies_tools_when_runtime_skill_cache_is_cold(monkeypatch):
    from unittest.mock import MagicMock
    ...
    monkeypatch.setattr(prompt_module, "_ensure_enabled_skills_cache", lambda: None)
    monkeypatch.setattr(lead_agent_module, "get_app_config", lambda: mock_app_config)
...
def test_make_lead_agent_uses_cached_skills_without_storage_scan(monkeypatch):
    from unittest.mock import MagicMock
    ...
    with prompt_module._enabled_skills_lock:
        prompt_module._enabled_skills_cache = [_make_skill("restricted", ["read_file"])]
        prompt_module._enabled_skills_by_config_cache.clear()

    def fail_storage_scan(*args, **kwargs):
        raise AssertionError("graph factory must not synchronously scan skill storage")

    monkeypatch.setattr(prompt_module, "get_or_new_skill_storage", fail_storage_scan)

    agent_kwargs = lead_agent_module.make_lead_agent({"configurable": {"agent_name": "test"}})

    assert [tool.name for tool in agent_kwargs["tools"]] == ["read_file"]
    assert "restricted" in agent_kwargs["system_prompt"]
...
def test_make_lead_agent_fails_closed_when_skill_policy_load_fails(monkeypatch):
    ...
    def fail_runtime_skills(*args, **kwargs):
        raise RuntimeError("skill storage unavailable")

    monkeypatch.setattr(prompt_module, "get_runtime_enabled_skills", fail_runtime_skills)
    monkeypatch.setattr(lead_agent_module, "get_app_config", lambda: mock_app_config)
